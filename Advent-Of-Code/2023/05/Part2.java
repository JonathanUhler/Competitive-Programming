...
public class Part2 {
	...
	public static void main(String[] args) throws Exception {
		...
		for (int i = 0; i < seedsDef.length; i += 2) {
			long seedStart = Long.parseLong(seedsDef[i]);
			long seedLen = Long.parseLong(seedsDef[i + 1]);

			for (long seed = seedStart; seed < seedStart + seedLen; seed++) {
				if (seed % 1_000_000 == 0)
					System.out.println(i + " / " + seedsDef.length + ":\t" + ((double)seed / (seedStart + seedLen)) + "\t-\t" + ans);
